...
def estimate_next_pos(measurement, OTHER = None):
    ...
    if OTHER==None:
        ...
        OTHER.append([]) ##for every (x,y,r)
        OTHER.append([]) ##for every step_dist
        OTHER.append(False) ##for clockwise
        OTHER.append([]) ##for aver x ,y ,r ,step_size
        xy_estimate=measurement
        return xy_estimate, OTHER 
    elif len(OTHER[0])<30:
        ##collect enough measurements to obtain the center
...
def next_move(hunter_position, hunter_heading, target_measurement, max_distance, OTHER = None):
    ...
    if OTHER==None: ##first measurement
        ...
        if len(OTHER[0][4])==0:    ##when no aver center and aver step_distance
            ...
            aver_cen_step=OTHER[0][4] ##average Xc, Xy, r and step_dist from estimate_next_pos
            crosstrack_error=cte(aver_cen_step[2],[aver_cen_step[0],aver_cen_step[1]],hunter_position)

            diff_crosstrack_error=-OTHER[1] ##OTHER[1]: last cte 
            ...
            if dist>max_distance:
                ...
                if OTHER[0][3]==False: ##if the bot moves counterclockwisely
...
target = robot(10.0, 10.0, 0.0, 2*pi / 30, 1.5)
measurement_noise = .05*target.distance
target.set_noise(0.0, 0.0, measurement_noise)

hunter = robot(0.0, 0.0, 0.0)
