...
def estimate_next_pos(measurement, OTHER = None):
    ...
    if OTHER==None:
        OTHER=[]
        OTHER.append(measurement) ## OTHER[0] for next measurement as previous measurement
        xy_estimate=measurement
    elif len(OTHER)==1:
        distance=distance_between(measurement,OTHER[0])
        heading=atan2(measurement[1]-OTHER[0][1],measurement[0]-OTHER[0][0])
        OTHER.append(heading) ## OTEHER[1] heading from the fist position to the second
        OTHER[0]=measurement ## update OTHER[0]
        xy_estimate=measurement
    else:
        distance=distance_between(measurement,OTHER[0]) 
        heading=atan2(measurement[1]-OTHER[0][1],measurement[0]-OTHER[0][0]) ##get the current heading direction
        turning=heading-OTHER[1] 
        OTHER=[measurement,heading] ## update OTHER for next measurement and next guess if it has to
        new_heading=heading+turning
        g_x=measurement[0]+cos(new_heading)*distance
        g_y=measurement[1]+sin(new_heading)*distance
        xy_estimate=(g_x,g_y)
